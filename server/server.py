...
async def create_match( currLevel, sid ):
    if len( active_students[currLevel]) > 1:
        ...
        all_rooms[currRoom] = ( player_1, player_2)
        ...
        await sio.emit( "response", "SERVER: Kindly wait for sometime, we will find a pair for you to play", room=sid)
...
@sio.event
async def update_apple( sid, data):
    await sio.emit("update_apple", {
        "APPLE_POS": create_apple()
    }, room=data['room'])
...
async def game_over( sid, data):
    room = data["room"]
    loser = data["loser"]
    winner = all_rooms[room][0] if all_rooms[room][1] == loser else all_rooms[room][1]
    ...
    all_students[winner]["SCORE"] += 2
    all_students[winner]["LEVEL"] += 1
    active_students[all_students[winner]["LEVEL"]].append(winner)

    try:
        await create_match( all_students[winner]["LEVEL"], winner )
    except Exception as e:
        print("Some thing went wrong:", e)


    print("****************************** Current Board ******************************")
    for _,s in all_students:
        mailId = s["MAIL_ID"]
        score = s["SOCRE"]
        print(f"Syudent:    {mailId}    {score}")
...
# DONE # Most implortant the common spot of the apple to be created.
