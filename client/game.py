...
def create_apple( c, apple_pos = None):
    ...
    APPLE = c.create_oval(apple_posx, apple_posy, apple_posx + SEG_SIZE, apple_posy + SEG_SIZE, fill="orange")
...
class Snake(object):
    ...
    def add_segment(self, color=None):
        ...
        self.segments.insert(0, Segment( self.c, x, y, color))
    ...
    def updateSnake(self, positions):
        # Remove all red-colored elements from the canvas
        for item in self.c.find_all():
            if self.c.itemcget(item, "fill") == "red":
                self.c.delete(item)
...
def main():
    ...
    if(
        ...
        IN_GAME = False
        print("IIIIIII HHHHHAAAVVEEE LLLOOOOSSSTTT!!!!")
        return {
            "STATUS": "GAME-OVER"
        }
    ...
    # I need to comment this part
    # if check_boundary_collision(currSnakeCoors):
    #     x1, y1, x2, y2 = currSnakeCoors
    #     if max(x1,x2) >= WIDTH - 20:
    #         currSnake.change_direction_ai("Left")
            
    #     if min(x1,x2) <= 20:
    #         currSnake.change_direction_ai("Right")
            
    #     if max(y1,y2) >= HEIGHT - 20:
    #         currSnake.change_direction_ai("Up")
            
    #     if min(y1,y2) <= 20:
    #         currSnake.change_direction_ai("Down")
    # else:
    #     if random.randint(1, 10) == 1:
    #         direction = random.choice(["Up", "Down", "Right", "Left"])
    #         currSnake.change_direction_ai(direction)
...
def insert_apple( apple_pos ):
    global c
    try:
        # Delete the apple if at all there is one:
        c.delete(APPLE)
    except Exception as e:
        print("There is no Apple in this game to delete.")
